...
#define ANCHOR_TWR_LOG_TAG "ANCHOR"
...
void anchor_main(void) {
    ...
    {
        ESP_LOGE(ANCHOR_TWR_LOG_TAG, "IDLE FAILED");
        while (1) ;
    }
    ...
    {
        ESP_LOGE(ANCHOR_TWR_LOG_TAG, "INIT FAILED");
        while (1) ;
    }
    ...
    {
        ESP_LOGE(ANCHOR_TWR_LOG_TAG, "CONFIG FAILED");
        while (1) ;
    }
    ...
    ESP_LOGI(ANCHOR_TWR_LOG_TAG, "Anchor Initialization complete");
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            if (frame_len <= sizeof(rx_buffer))
            {
                ...
                if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
                {
                    ...
                    {
                        ESP_LOGE(ANCHOR_TWR_LOG_TAG, "TX ERROR");
                    }
                }
            }
        }
